    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="bg-white rounded-2xl shadow-lg mb-8 p-8">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
                <div class="space-y-2">
                    ...
                    <p class="text-gray-500">Kelola semua jenis kegiatan dalam satu dashboard terintegrasi</p>
                </div>
                <button data-modal-target="addJenisKegiatanModal" data-modal-toggle="addJenisKegiatanModal"
        ...
        <div class="bg-white rounded-2xl shadow-lg mb-8 p-6">
            <div class="flex flex-col sm:flex-row gap-4">
                <div class="flex-1">
                    <div class="relative">
                        <input type="text" id="searchInput" placeholder="Cari jenis kegiatan..."
                ...
                <table class="w-full">
                    <thead>
                        <tr class="bg-gray-50">
                            ...
                            <th class="px-6 py-4 text-xs font-semibold text-gray-600 uppercase tracking-wider text-left">Jenis Kegiatan</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr data-id="{{ $item->id_jenis_kegiatan }}" class="hover:bg-gray-50 transition-colors duration-200">
                            <td class="px-6 py-4 text-sm text-gray-800">{{ $key + 1 }}</td>
                            <td class="px-6 py-4">
                                <div class="text-sm font-medium text-gray-900">{{ $item->nama_jenis_kegiatan }}</div>
                            </td>
                            ...
                            <td class="px-6 py-4 text-center">
                                <button onclick="openEditModal('{{ $item->id_jenis_kegiatan }}', '{{ $item->nama_jenis_kegiatan }}', '{{ $item->status }}')"
        ...
        <!-- Add Modal -->
        <div id="addJenisKegiatanModal" tabindex="-1" class="fixed inset-0 flex items-center justify-center hidden z-50">
                ...
                <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
                    ...
                    <form id="jenisKegiatanForm" action="{{ route('jenis-kegiatan.store') }}" method="POST">
                        ...
                        <div class="px-6 py-4 space-y-4">
                            <div>
                                <label for="nama_jenis_kegiatan" class="block text-sm font-medium text-gray-700 mb-2">
                                    Nama Jenis Kegiatan
                                </label>
                                <input type="text" name="nama_jenis_kegiatan" id="nama_jenis_kegiatan" required
        ...
        <div id="editModal" tabindex="-1" class="fixed inset-0 flex items-center justify-center hidden z-50">
            ...
            <div class="relative w-full max-w-md transform transition-all">
                <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
                    ...
                    <!-- Modal Form -->
                    <form id="editJenisKegiatanForm" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="px-6 py-4 space-y-4">
                            <div>
                                <label for="edit_nama_jenis_kegiatan" class="block text-sm font-medium text-gray-700 mb-2">
                                    Nama Jenis Kegiatan
                                </label>
                                <input type="text" name="nama_jenis_kegiatan" id="edit_nama_jenis_kegiatan" required
                                    class="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200">
                            </div>
                            <div>
    ...
    if (searchInput) {
        searchInput.addEventListener('keyup', function() {
            ...
            tableRows.forEach(row => {
                ...
                const shouldShow = activityName.includes(searchTerm);
...
function showNotification(message, type = 'success') {
    ...
    } transform transition-all duration-300 translate-y-0 opacity-0 z-50`;
